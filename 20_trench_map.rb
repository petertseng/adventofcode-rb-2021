...
def enhance_4x4(enhance_rule_4x4, width, read, write, active_height, active_width)
  (0...active_height).each { |y|
    ...
    (0...active_width).each { |x|
      ...
      # moving right is shifting left, becauser lower-order bits need to become higher-order bits when moving right.
...
grid = ARGF.each_line(chomp: true).map { |line|
  line.each_char.map(&method(:bit)).freeze
}.freeze
